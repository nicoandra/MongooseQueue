...
class MongooseQueue
{
	...
	constructor(payloadModel, workerId = '', options = {})
	{
		...
		this.jobStats = { added: 0, done: 0, effectiveness: 0, donePerPeriod: 0, addedPerPeriod : 0, effectivenessPerPeriod : 0 };
	}
	...
	/**
	 *  Calls the callback with a set of statuses and the count of elements$group
	 * @param {fn(err, result)} cb - Callback with null and an object with data when successful , otherwise the error is passed.
	 */
	getStats(cb){
		this.JobModel.aggregate([{ $group : { _id : "$doneNum" , count : { $sum : 1 }} }]).exec(function(err, res){
			if(err){
				return cb(err, null);
			}

			res = { res : res , perf : jobStats }
			return cb(null, res);
		});
	}
}
